...
public class A04_PreparedDao {
	...
	*/
	...
	public List<Department> getDept() {
	    List<Department> elist = new ArrayList<Department>();
	    String sql = "	SELECT * \r\n"
	    		+ "	FROM Departments10\r\n";
	    System.out.println("# DB 접속 #");
	    try {
	...
	*/
	...
	public Job getJob1(String job_id) {
	    Job j = new Job("","",0,0);
	    String sql = "	SELECT * \r\n"
	    		+ "	FROM jobs\r\n"
	    		+ "	where job_id LIKE ? ";
	...
	public List<Job> getJob2(String job_id) {
	...
	public Salgrade getSal(int grade){
		Salgrade s = new Salgrade(0,0,0);
		String sql = "SELECT * FROM SALGRADE s WHERE grade = ?";
	    try {
	        con = DB.con();
	        pstmt = con.prepareStatement(sql);
	...
	//int empno, String ename, String job, Double sal
	public Emp getEmp(int no){
		Emp em = null;
		String sql = "SELECT * FROM emp s WHERE empno = ?";
	    try {
	        con = DB.con();
	        pstmt = con.prepareStatement(sql);
	        pstmt.setInt(1, no);
