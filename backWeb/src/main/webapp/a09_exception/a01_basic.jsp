...
1. 웹 개발 프로그래밍을 하면 에러가 발생할 수 있다.
	자바 프로그램에서 이러한 에러가 발생되면 예외처리로 해당 에러에 대한 내용의 원인을 출력하고,
	에러가 발생하더라도 프로세스가 멈추지 않게 한다.
2. 웹 개발에서는 위 내용과 함께 jsp화면에서 에러에 대하여 어떻게 표현하고ㅡ
	처리해주는 것까지 포함된다.
3. 에러처리의 방식
...
2. 에러가 발생할 가능성이 있는 페이지에 위 에러 처리페이지 설정
	<%@ page errorPage = "위 만들어둔 에러페이지 지정"%>
	
#	서버 단위로 에러 페이지 지정 구현
0. 위와 같이 모든 페이지에 설정을 하지 않더라도 효과적으로 페이지를 대체할 수 있다.
...
2. http 코드 에러로 설정
	...
	<error-page>
		...
		webapp기준으로 하위에 특정 경로의 에러페이지를 지정하여 처리
	2) 응답코드종류
		200 : 요청이 성공적으로 처리
